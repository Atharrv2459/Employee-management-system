...
export default function ManagerTimesheet() {
  ...
  // ✅ Helper function to convert minutes to "0h 0m" format
  ...
  const fetchTimesheetData = async () => {
    try {
      ...
      const currentPeriod = raw.filter(
        (entry) => entry.punch_in && new Date(entry.punch_in).getMonth() === now.getMonth()
      );
      ...
      const attendancePercent = Math.round((presentDays.size / workingDays) * 100);
